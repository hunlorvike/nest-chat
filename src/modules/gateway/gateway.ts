...
import { Services, WebsocketEvents } from "src/common/utils/constrants";
import { IGatewayJWTManager } from "./gateway.jwt";
...
import { IGroupService } from "../group/services/interface-group.service";
import { IFriendService } from "../friend/services/interface-friend.service";
import { OnEvent } from "@nestjs/event-emitter";
import { AddGroupUserResponse, CallAcceptedPayload, CallHangUpPayload, CreateGroupMessageResponse, CreateMessageResponse, RemoveGroupUserResponse, VoiceCallPayload } from "src/common/utils/types";
import { Conversation } from "../conversations/entities/conversation.entity";
import { Message } from "../message/entities/message.entity";
import { Group } from "../group/entities/group.entity";
import { GroupMessage } from "../group/entities/group-message.entity";
...
export class MessagingGateway implements OnGatewayConnection, OnGatewayDisconnect {
    ...
    constructor(
        @Inject(Services.GATEWAY_JWT_MANAGER) readonly jwtManager: IGatewayJWTManager,
    ...
    handleConnection(socket: AuthenticatedSocket, ...args: any[]) {
        ...
        this.jwtManager.setUserSocket(socket.user.id, socket);
    ...
    handleDisconnect(socket: AuthenticatedSocket) {
        ...
        this.jwtManager.removeUserSocket(socket.user.id);
    }
    ...
    async handleGetOnlineGroupUsers(
        ...
        group.users.forEach((user) => {
            const socket = this.jwtManager.getUserSocket(user.id);
    ...
    handleMessageCreateEvent(payload: CreateMessageResponse) {
        ...
        const authorSocket = this.jwtManager.getUserSocket(author.id);
        const recipientSocket =
            author.id === creator.id
                ? this.jwtManager.getUserSocket(recipient.id)
                : this.jwtManager.getUserSocket(creator.id);
    ...
    handleConversationCreateEvent(payload: Conversation) {
        ...
        const recipientSocket = this.jwtManager.getUserSocket(payload.recipient.id);
    ...
    handleGroupUserAdd(payload: AddGroupUserResponse) {
        const recipientSocket = this.jwtManager.getUserSocket(payload.user.id);
    ...
    handleGroupUserRemove(payload: RemoveGroupUserResponse) {
        ...
        const removedUserSocket = this.jwtManager.getUserSocket(payload.user.id);
        console.log(payload);
        ...
        const onlineUsers = group.users
            .map((user) => this.jwtManager.getUserSocket(user.id) && user)
            .filter((user) => user);
    ...
    handleGroupOwnerUpdate(payload: Group) {
        ...
        const newOwnerSocket = this.jwtManager.getUserSocket(payload.owner.id);
    ...
    handleGroupUserLeave(payload) {
        ...
        const { rooms } = this.server.sockets.adapter;
        const socketsInRoom = rooms.get(ROOM_NAME);
        const leftUserSocket = this.jwtManager.getUserSocket(payload.userId);
        /**
    ...
    async handleVideoCall(
        ...
        const receiverSocket = this.jwtManager.getUserSocket(data.recipientId);
    ...
    async handleVideoCallAccepted(
        ...
        const callerSocket = this.jwtManager.getUserSocket(data.caller.id);
    ...
    async handleVideoCallHangUp(
        ...
        if (socket.user.id === caller.id) {
            const receiverSocket = this.jwtManager.getUserSocket(receiver.id);
            socket.emit('onVideoCallHangUp');
            return receiverSocket && receiverSocket.emit('onVideoCallHangUp');
        }
        socket.emit('onVideoCallHangUp');
        const callerSocket = this.jwtManager.getUserSocket(caller.id);
    ...
    async handleVoiceCallInitiate(
        ...
        const caller = socket.user;
        const receiverSocket = this.jwtManager.getUserSocket(payload.recipientId);
        if (!receiverSocket) socket.emit('onUserUnavailable');
    ...
    async handleVoiceCallAccepted(
        ...
        const callerSocket = this.jwtManager.getUserSocket(payload.caller.id);
        const conversation = await this.conversationService.isCreated(
            payload.caller.id,
            socket.user.id,
        );
